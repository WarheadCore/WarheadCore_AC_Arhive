...
bool WorldSocketMgr::StartWorldNetwork(Warhead::Asio::IoContext& ioContext, std::string const& bindIp, uint16 port, int threadCount)
{
    ...
    WC_LOG_DEBUG("misc", "Max allowed socket connections %d", max_connections);
    ...
    {
        WC_LOG_ERROR("misc", "Network.OutUBuff is wrong in your config file");
        return false;
    }
...
void WorldSocketMgr::OnSocketOpen(tcp::socket&& sock, uint32 threadIndex)
{
    ...
    if (_socketSystemSendBufferSize >= 0)
    {
        ...
        {
            WC_LOG_ERROR("misc", "WorldSocketMgr::OnSocketOpen sock.set_option(boost::asio::socket_base::send_buffer_size) err = %s", err.message().c_str());
            return;
        }
    }
    ...
    if (_tcpNoDelay)
    {
        ...
        if (err)
        {
            WC_LOG_ERROR("misc", "WorldSocketMgr::OnSocketOpen sock.set_option(boost::asio::ip::tcp::no_delay) err = %s", err.message().c_str());
            return;
        }
    }
