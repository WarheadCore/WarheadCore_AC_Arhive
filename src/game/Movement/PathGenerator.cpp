    ...
    _navMeshQuery(nullptr)
{
    ...
    WC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::PathGenerator for %u", _sourceUnit->GetGUID().GetCounter());
...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ, bool forceDest, bool straightLine)
{
    ...
    WC_METRIC_EVENT("mmap_events", "CalculatePath", "");
    ...
    WC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::CalculatePath() for %u", _sourceUnit->GetGUID().GetCounter());
...
void PathGenerator::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
    ...
    {
        WC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: (startPoly == 0 || endPoly == 0)");
        BuildShortcut();
    ...
    if (farFromPoly)
    {
        WC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: farFromPoly distToStartPoly=%.3f distToEndPoly=%.3f", distToStartPoly, distToEndPoly);
        ...
        if (_sourceUnit->GetTypeId() == TYPEID_UNIT)
        {
            ...
            {
                WC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: underWater case");
                if (owner->CanSwim())
                    buildShotrcut = true;
            }
            else
            {
                WC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: flying case");
                if (owner->CanFly())
                    buildShotrcut = true;
            }
        }
    ...
    {
        WC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: (startPoly == endPoly)");
        ...
        WC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: path type %d", _type);
        return;
    }
    ...
    if (_polyLength)
    {
        for (; pathStartIndex < _polyLength; ++pathStartIndex)
        {
            ...
            {
                WC_LOG_ERROR("maps.mmaps", "Invalid poly ref in BuildPolyPath. _polyLength: %u, pathStartIndex: %u,"
    ...
    {
        WC_LOG_DEBUG("maps.mmaps", "++ BuildPolyPath :: (startPolyFound && endPolyFound)");
    ...
    else if (startPolyFound && !endPolyFound)
    {
        ...
        if (!suffixPolyLength || dtStatusFailed(dtResult))
        {
            ...
            WC_LOG_ERROR("maps.mmaps", "Path Build failed\n%s", _sourceUnit->GetDebugInfo().c_str());
        }

        WC_LOG_DEBUG("maps.mmaps", "++  m_polyLength=%u prefixPolyLength=%u suffixPolyLength=%u", _polyLength, prefixPolyLength, suffixPolyLength);
    ...
    else
    {
        ...
        if (!_polyLength || dtStatusFailed(dtResult))
        {
            ...
            WC_LOG_ERROR("maps.mmaps", "%u's Path Build failed: 0 length path", _sourceUnit->GetGUID().GetCounter());
            BuildShortcut();
            _type = PATHFIND_NOPATH;
            return;
        }
...
void PathGenerator::BuildPointPath(const float *startPoint, const float *endPoint)
{
    ...
    if (pointCount < 2 || dtStatusFailed(dtResult))
    {
        ...
        WC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::BuildPointPath FAILED! path sized %d returned\n", pointCount);
        BuildShortcut();
    ...
    {
        WC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::BuildPointPath FAILED! path sized %d returned, lower than limit set to %d", pointCount, _pointPathLimit);
        BuildShortcut();
    ...
    WC_LOG_DEBUG("maps.mmaps", "++ PathGenerator::BuildPointPath path type %d size %d poly-size %d", _type, pointCount, _polyLength);
}
...
{
    WC_LOG_DEBUG("maps.mmaps", "++ BuildShortcut :: making shortcut");
...
void PathGenerator::ShortenPathUntilDist(G3D::Vector3 const& target, float dist)
{
    ...
    {
        WC_LOG_ERROR("maps.mmaps", "PathGenerator::ReducePathLengthByDist called before path was successfully built");
        return;
    }
