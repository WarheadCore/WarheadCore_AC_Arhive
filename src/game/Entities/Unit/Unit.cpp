...
void Unit::DealSpellDamage(SpellNonMeleeDamage* damageInfo, bool durabilityLoss)
{
    ...
    {
        WC_LOG_DEBUG("entities.unit", "Unit::DealSpellDamage has wrong damageInfo->SpellID: %u", damageInfo->SpellID);
        return;
    }
...
void Unit::AttackerStateUpdate(Unit* victim, WeaponAttackType attType, bool extra)
{
    ...
    else
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
            WC_LOG_DEBUG("entities.unit", "AttackerStateUpdate: (Player) %u attacked %u (TypeId: %u) for %u dmg, absorbed %u, blocked %u, resisted %u.",
            ...
            WC_LOG_DEBUG("entities.unit", "AttackerStateUpdate: (NPC)    %u attacked %u (TypeId: %u) for %u dmg, absorbed %u, blocked %u, resisted %u.",
                GetGUID().GetCounter(), victim->GetGUID().GetCounter(), victim->GetTypeId(), dmgInfo.GetDamage(), dmgInfo.GetAbsorb(), dmgInfo.GetBlock(), dmgInfo.GetResist());
    }
}
...
void Unit::SendMeleeAttackStart(Unit* victim)
{
    ...
    data << uint64(victim->GetGUID());
    SendMessageToSet(&data, true);
    WC_LOG_DEBUG("entities.unit", "WORLD: Sent SMSG_ATTACKSTART");
}
...
void Unit::SendMeleeAttackStop(Unit* victim)
{
    ...
    SendMessageToSet(&data, true);
    WC_LOG_DEBUG("entities.unit", "WORLD: Sent SMSG_ATTACKSTOP");

    if (victim)
        WC_LOG_DEBUG("entities.unit", "%s %u stopped attacking %s %u", (GetTypeId() == TYPEID_PLAYER ? "Player" : "Creature"), GetGUID().GetCounter(), (victim->GetTypeId() == TYPEID_PLAYER ? "player" : "creature"), victim->GetGUID().GetCounter());
    else
        WC_LOG_DEBUG("entities.unit", "%s %u stopped attacking", (GetTypeId() == TYPEID_PLAYER ? "Player" : "Creature"), GetGUID().GetCounter());
}
    ...
    for (AuraEffect const* aurEff : ignore)
    {
        ...
        switch (aurEff->GetMiscValue())
        {
            ...
            case MELEE_HIT_PARRY:
                ...
                break;
            default:
                WC_LOG_DEBUG("entities.unit", "Spell %u SPELL_AURA_IGNORE_COMBAT_RESULT has unhandled state %d", aurEff->GetId(), aurEff->GetMiscValue());
                break;
        }
    }
...
{
    //WC_LOG_DEBUG("entities.unit", "Interrupt spell for unit %u.", GetEntry());
...
AuraApplication* Unit::_CreateAuraApplication(Aura* aura, uint8 effMask)
{
    ...
    if (aura->IsRemoved())
    {
        WC_LOG_ERROR("spells", "Unit::_CreateAuraApplication() called with a removed aura. Check if OnEffectHitTarget() is triggering any spell with apply aura effect (that's not allowed!)\nUnit: %s\nAura: %s", GetDebugInfo().c_str(), aura->GetDebugInfo().c_str());
        return nullptr;
    }
...
void Unit::_UnapplyAura(AuraApplicationMap::iterator& i, AuraRemoveMode removeMode)
{
    ...
    Aura* aura = aurApp->GetBase();
    WC_LOG_DEBUG("spells", "Aura %u now is remove mode %d", aura->GetId(), removeMode);
...
void Unit::RemoveOwnedAura(Aura* aura, AuraRemoveMode removeMode)
{
    ...
    {
        WC_LOG_ERROR("spells", "Unit::RemoveOwnedAura() called with unallowed removeMode AURA_REMOVE_NONE, spellId %u", aura->GetId());
        return;
    }
...
void Unit::SendPeriodicAuraLog(SpellPeriodicAuraLogInfo* pInfo)
{
    ...
    switch (aura->GetAuraType())
    {
        ...
        case SPELL_AURA_PERIODIC_MANA_LEECH:
            ...
            break;
        default:
            WC_LOG_ERROR("entities.unit", "Unit::SendPeriodicAuraLog: unknown aura %u", uint32(aura->GetAuraType()));
            return;
    }
...
void Unit::RemoveAllAttackers()
{
    while (!m_attackers.empty())
    {
        ...
        if (!(*iter)->AttackStop())
        {
            WC_LOG_ERROR("entities.unit", "WORLD: Unit has an attacker that isn't attacking it!");
...
Minion* Unit::GetFirstMinion() const
{
    if (ObjectGuid pet_guid = GetMinionGUID())
    {
        ...
        WC_LOG_ERROR("entities.unit", "Unit::GetFirstMinion: Minion %s not exist.", pet_guid.ToString().c_str());
...
Guardian* Unit::GetGuardianPet() const
{
    if (ObjectGuid pet_guid = GetPetGUID())
    {
        ...
        WC_LOG_FATAL("entities.unit", "Unit::GetGuardianPet: Guardian %s not exist.", pet_guid.ToString().c_str());
...
{
    WC_LOG_DEBUG("entities.unit", "SetMinion %u for %u, apply %u", minion->GetEntry(), GetEntry(), apply);

    if (apply)
    {
        if (minion->GetOwnerGUID())
        {
            WC_LOG_FATAL("entities.unit", "SetMinion: Minion %u is not the minion of owner %u", minion->GetEntry(), GetEntry());
            return;
        }
...
void Unit::RemoveAllControlled()
{
    ...
    while (!m_Controlled.empty())
    {
        ...
        if (target->GetCharmerGUID() == GetGUID())
            ...
            WC_LOG_ERROR("entities.unit", "Unit %u is trying to release unit %u which is neither charmed nor owned by it", GetEntry(), target->GetEntry());
    }
    if (GetPetGUID())
        WC_LOG_FATAL("entities.unit", "Unit %u is not able to release its pet %s", GetEntry(), GetPetGUID().ToString().c_str());
    if (GetMinionGUID())
        WC_LOG_FATAL("entities.unit", "Unit %u is not able to release its minion %s", GetEntry(), GetMinionGUID().ToString().c_str());
    if (GetCharmedGUID())
        WC_LOG_FATAL("entities.unit", "Unit %u is not able to release its charm %s", GetEntry(), GetCharmedGUID().ToString().c_str());
...
void Unit::UpdateSpeed(UnitMoveType mtype)
{
    ...
    switch (mtype)
    {
        ...
        default:
            WC_LOG_ERROR("entities.unit", "Unit::UpdateSpeed: Unsupported move type (%d)", mtype);
            return;
    }
...
void Unit::HandleStatFlatModifier(UnitMods unitMod, UnitModifierFlatType modifierType, float amount, bool apply)
{
    ...
    {
        WC_LOG_ERROR("entities.unit", "ERROR in HandleStatFlatModifier(): non-existing UnitMods or wrong UnitModifierType!");
        return;
    }
...
void Unit::ApplyStatPctModifier(UnitMods unitMod, UnitModifierPctType modifierType, float pct)
{
    ...
    {
        WC_LOG_ERROR("entities.unit", "ERROR in ApplyStatPctModifier(): non-existing UnitMods or wrong UnitModifierType!");
        return;
    }
...
float Unit::GetFlatModifierValue(UnitMods unitMod, UnitModifierFlatType modifierType) const
{
    if (unitMod >= UNIT_MOD_END || modifierType >= MODIFIER_TYPE_FLAT_END)
    {
        WC_LOG_ERROR("entities.unit", "attempt to access non-existing modifier value from UnitMods!");
        return 0.0f;
    }
...
float Unit::GetTotalAuraModValue(UnitMods unitMod) const
{
    ...
    {
        WC_LOG_ERROR("entities.unit", "attempt to access non-existing UnitMods in GetTotalAuraModValue()!");
        return 0.0f;
    }
...
void Unit::RemoveFromWorld()
{
    ...
    if (IsInWorld())
    {
        ...
        if (Unit* owner = GetOwner())
        {
            ...
            {
                WC_LOG_FATAL("entities.unit", "Unit %u is in controlled list of %u when removed from world", GetEntry(), owner->GetEntry());
                ABORT();
            }
        }
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            if (Unit* charmer = GetCharmer())
            {
                ...
                if (Creature* creatureCharmer = charmer->ToCreature())
                {
                    ...
                    WC_LOG_ERROR("entities.unit.ai", "Attempt to assign charm AI to player %s who is charmed by non-creature %s.", GetGUID().ToString().c_str(), GetCharmerGUID().ToString().c_str());
            }
...
bool Unit::InitTamedPet(Pet* pet, uint8 level, uint32 spell_id)
{
    ...
    {
        WC_LOG_ERROR("entities.unit", "Pet::InitStatsForLevel() failed for creature (Entry: %u)!", pet->GetEntry());
        return false;
    }
...
/*static*/ void Unit::Kill(Unit* attacker, Unit* victim, bool durabilityLoss /*= true*/)
{
    ...
    {
        WC_LOG_DEBUG("entities.unit", "SET JUST_DIED");
    ...
    if (player)
    {
        ...
        if (pet && pet->IsAlive() && pet->isControlled())
        {
            if (pet->IsAIEnabled())
                ...
                WC_LOG_ERROR("entities.unit", "Pet doesn't have any AI in Unit::Kill(). %s", pet->GetDebugInfo().c_str());
        }
    }
    ...
    if (Player* plrVictim = victim->ToPlayer())
    {
        ...
        {
            WC_LOG_DEBUG("entities.unit", "We are dead, losing %f percent durability", sWorld->getRate(RATE_DURABILITY_LOSS_ON_DEATH));
    ...
    {
        WC_LOG_DEBUG("entities.unit", "DealDamageNotPlayer");
...
bool Unit::SetCharmedBy(Unit* charmer, CharmType type, AuraApplication const* aurApp)
{
    ...
    WC_LOG_DEBUG("entities.unit", "SetCharmedBy: charmer %u (GUID %u), charmed %u (GUID %u), type %u.", charmer->GetEntry(), charmer->GetGUID().GetCounter(), GetEntry(), GetGUID().GetCounter(), uint32(type));

    if (this == charmer)
    {
        WC_LOG_FATAL("entities.unit", "Unit::SetCharmedBy: Unit %u (GUID %u) is trying to charm itself!", GetEntry(), GetGUID().GetCounter());
        return false;
    }
    ...
    {
        WC_LOG_FATAL("entities.unit", "Unit::SetCharmedBy: Player on transport is trying to charm %u (GUID %u)", GetEntry(), GetGUID().GetCounter());
        return false;
    }
    ...
    if (GetCharmerGUID())
    {
        WC_LOG_FATAL("entities.unit", "Unit::SetCharmedBy: %u (GUID %u) has already been charmed but %u (GUID %u) is trying to charm it!", GetEntry(), GetGUID().GetCounter(), charmer->GetEntry(), charmer->GetGUID().GetCounter());
        return false;
    }
    ...
    if (!IsInWorld())
    {
        WC_LOG_FATAL("entities.unit", "Unit::SetCharmedBy: %u (GUID %u) is not in world but %u (GUID %u) is trying to charm it!", GetEntry(), GetGUID().GetCounter(), charmer->GetEntry(), charmer->GetGUID().GetCounter());
        return false;
    }
                ...
                if (GetTypeId() == TYPEID_UNIT && charmer->getClass() == CLASS_WARLOCK)
                {
                    ...
                    if (cinfo && cinfo->type == CREATURE_TYPE_DEMON)
                    {
                        ...
                        if (GetCharmInfo())
                            ...
                            WC_LOG_ERROR("entities.unit", "Aura::HandleModCharm: %s has a charm aura but no charm info!", GetGUID().ToString().c_str());
                    }
                }
                break;
            case CHARM_TYPE_CONVERT:
...
void Unit::HandleSpellClick(Unit* clicker, int8 seatId /*= -1*/)
{
    ...
    for (auto const& clickPair : clickBounds)
    {
        ...
        if (seatId > -1)
        {
            ...
            if (!valid)
            {
                WC_LOG_ERROR("sql.sql", "Spell %u specified in npc_spellclick_spells is not a valid vehicle enter aura!", clickPair.second.spellId);
                continue;
            }
...
void Unit::_EnterVehicle(Vehicle* vehicle, int8 seatId, AuraApplication const* aurApp)
{
    ...
    if (m_vehicle)
    {
        ...
        {
            WC_LOG_DEBUG("entities.vehicle", "EnterVehicle: %u exit %u and enter %u.", GetEntry(), m_vehicle->GetBase()->GetEntry(), vehicle->GetBase()->GetEntry());
            ExitVehicle();
        }
...
bool Unit::UpdatePosition(float x, float y, float z, float orientation, bool teleport)
{
    ...
    {
        WC_LOG_DEBUG("entities.unit", "Unit::UpdatePosition(%f, %f, %f) .. bad coordinates!", x, y, z);
        return false;
    }
...
{
    WC_LOG_ERROR("entities.unit", "Unit::OutDebugInfo");
    WC_LOG_DEBUG("entities.unit", "%s name %s", GetGUID().ToString().c_str(), GetName().c_str());
    WC_LOG_DEBUG("entities.unit", "Owner %s, Minion %s, Charmer %s, Charmed %s", GetOwnerGUID().ToString().c_str(), GetMinionGUID().ToString().c_str(), GetCharmerGUID().ToString().c_str(), GetCharmedGUID().ToString().c_str());
    WC_LOG_DEBUG("entities.unit", "In world %u, unit type mask %u", (uint32)(IsInWorld() ? 1 : 0), m_unitTypeMask);
    if (IsInWorld())
        WC_LOG_DEBUG("entities.unit", "Mapid %u", GetMapId());
    ...
    WC_LOG_DEBUG("entities.unit", "%s", o.str().c_str());
    o.str("");
    ...
    if (GetVehicle())
        WC_LOG_DEBUG("entities.unit", "On vehicle %u.", GetVehicleBase()->GetEntry());
}
...
void Unit::Talk(uint32 textId, ChatMsg msgType, float textRange, WorldObject const* target)
{
    ...
    {
        WC_LOG_ERROR("entities.unit", "WorldObject::MonsterText: `broadcast_text` (ID: %u) was not found", textId);
        return;
    }
