...
void WorldSession::HandleQuestgiverStatusQueryOpcode(WorldPacket& recvData)
{
    ...
    if (!questGiver)
    {
        WC_LOG_INFO("network", "Error in CMSG_QUESTGIVER_STATUS_QUERY, called for non-existing questgiver (%s)", guid.ToString().c_str());
        return;
    }
    ...
    switch (questGiver->GetTypeId())
    {
        case TYPEID_UNIT:
        {
            WC_LOG_DEBUG("network", "WORLD: Received CMSG_QUESTGIVER_STATUS_QUERY for npc, guid = %u", questGiver->GetGUID().GetCounter());
        ...
        {
            WC_LOG_DEBUG("network", "WORLD: Received CMSG_QUESTGIVER_STATUS_QUERY for GameObject guid = %u", questGiver->GetGUID().GetCounter());
            questStatus = _player->GetQuestDialogStatus(questGiver);
            break;
        }
        default:
            WC_LOG_ERROR("network", "QuestGiver called for unexpected type %u", questGiver->GetTypeId());
            break;
    }
...
void WorldSession::HandleQuestgiverHelloOpcode(WorldPacket& recvData)
{
    ...
    WC_LOG_DEBUG("network", "WORLD: Received CMSG_QUESTGIVER_HELLO %s", guid.ToString().c_str());

    Creature* creature = GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_QUESTGIVER);
    if (!creature)
    {
        WC_LOG_DEBUG("network", "WORLD: HandleQuestgiverHelloOpcode - %s not found or you can't interact with him.",
            guid.ToString().c_str());
        return;
    }
...
void WorldSession::HandleQuestgiverAcceptQuestOpcode(WorldPacket& recvData)
{
    ...
    WC_LOG_DEBUG("network", "WORLD: Received CMSG_QUESTGIVER_ACCEPT_QUEST %s, quest = %u, startCheat = %u", guid.ToString().c_str(), questId, startCheat);
...
void WorldSession::HandleQuestgiverQueryQuestOpcode(WorldPacket& recvData)
{
    ...
    WC_LOG_DEBUG("network", "WORLD: Received CMSG_QUESTGIVER_QUERY_QUEST npc = %s, quest = %u, unk1 = %u", guid.ToString().c_str(), questId, unk1);
...
{
    WC_LOG_DEBUG("network", "WORLD: Received CMSG_QUEST_QUERY quest = %u", query.QuestID);
...
void WorldSession::HandleQuestgiverChooseRewardOpcode(WorldPacket& recvData)
{
    ...
    {
        WC_LOG_ERROR("entities.player.cheat", "Error in CMSG_QUESTGIVER_CHOOSE_REWARD: player %s (guid %d) tried to get invalid reward (%u) (possible packet-hacking detected)", _player->GetName().c_str(), _player->GetGUID().GetCounter(), reward);
        return;
    }

    WC_LOG_DEBUG("network", "WORLD: Received CMSG_QUESTGIVER_CHOOSE_REWARD npc = %s, quest = %u, reward = %u", guid.ToString().c_str(), questId, reward);
    ...
    if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId))
    {
        ...
        {
            WC_LOG_ERROR("entities.player.cheat", "Error in QUEST_STATUS_COMPLETE: player %s (guid %u) tried to complete quest %u, but is not allowed to do so (possible packet-hacking or high latency)",
...
void WorldSession::HandleQuestgiverRequestRewardOpcode(WorldPacket& recvData)
{
    ...
    WC_LOG_DEBUG("network", "WORLD: Received CMSG_QUESTGIVER_REQUEST_REWARD npc = %s, quest = %u", guid.ToString().c_str(), questId);
...
{
    WC_LOG_DEBUG("network", "WORLD: Received CMSG_QUESTGIVER_CANCEL");
...
void WorldSession::HandleQuestLogSwapQuest(WorldPacket& recvData)
{
    ...
    WC_LOG_DEBUG("network", "WORLD: Received CMSG_QUESTLOG_SWAP_QUEST slot 1 = %u, slot 2 = %u", slot1, slot2);
...
void WorldSession::HandleQuestLogRemoveQuest(WorldPacket& recvData)
{
    ...
    WC_LOG_DEBUG("network", "WORLD: Received CMSG_QUESTLOG_REMOVE_QUEST slot = %u", slot);
    ...
    if (slot < MAX_QUEST_LOG_SIZE)
    {
        if (uint32 questId = _player->GetQuestSlotQuestId(slot))
        {
            ...
            WC_LOG_INFO("network", "Player %u abandoned quest %u", _player->GetGUID().GetCounter(), questId);
...
void WorldSession::HandleQuestConfirmAccept(WorldPacket& recvData)
{
    ...
    WC_LOG_DEBUG("network", "WORLD: Received CMSG_QUEST_CONFIRM_ACCEPT quest = %u", questId);
...
void WorldSession::HandleQuestgiverCompleteQuest(WorldPacket& recvData)
{
    ...
    WC_LOG_DEBUG("network", "WORLD: Received CMSG_QUESTGIVER_COMPLETE_QUEST npc = %s, quest = %u", guid.ToString().c_str(), questId);
    ...
    if (!_player->CanSeeStartQuest(quest) && _player->GetQuestStatus(questId) == QUEST_STATUS_NONE)
    {
        WC_LOG_ERROR("entities.player.cheat", "Possible hacking attempt: Player %s [guid: %u] tried to complete quest [entry: %u] without being in possession of the quest!",
                      _player->GetName().c_str(), _player->GetGUID().GetCounter(), questId);
        return;
    }
...
{
    WC_LOG_DEBUG("network", "WORLD: Received CMSG_QUESTGIVER_QUEST_AUTOLAUNCH");
}
...
void WorldSession::HandlePushQuestToParty(WorldPacket& recvPacket)
{
    ...
    WC_LOG_DEBUG("network", "WORLD: Received CMSG_PUSHQUESTTOPARTY questId = %u", questId);
...
void WorldSession::HandleQuestPushResult(WorldPacket& recvPacket)
{
    ...
    WC_LOG_DEBUG("network", "WORLD: Received MSG_QUEST_PUSH_RESULT");
...
{
    WC_LOG_DEBUG("network", "WORLD: Received CMSG_QUESTGIVER_STATUS_MULTIPLE_QUERY");
