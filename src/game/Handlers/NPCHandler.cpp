...
void WorldSession::HandleTabardVendorActivateOpcode(WorldPacket& recvData)
{
    ...
    if (!unit)
    {
        WC_LOG_DEBUG("network", "WORLD: HandleTabardVendorActivateOpcode - %s not found or you can not interact with him.", guid.ToString().c_str());
        return;
    }
...
void WorldSession::HandleBankerActivateOpcode(WorldPacket& recvData)
{
    ...
    WC_LOG_DEBUG("network", "WORLD: Received CMSG_BANKER_ACTIVATE");
    ...
    if (!unit)
    {
        WC_LOG_DEBUG("network", "WORLD: HandleBankerActivateOpcode - %s not found or you can not interact with him.", guid.ToString().c_str());
        return;
    }
...
void WorldSession::HandleTrainerListOpcode(WorldPackets::NPC::Hello& packet)
{
    ...
    if (!npc)
    {
        WC_LOG_DEBUG("network", "WorldSession: SendTrainerList - %s not found or you can not interact with him.", packet.Unit.ToString().c_str());
        return;
    }
...
void WorldSession::SendTrainerList(Creature* npc)
{
    ...
    if (!trainer)
    {
        WC_LOG_DEBUG("network", "WorldSession: SendTrainerList - trainer spells not found for %s", npc->GetGUID().ToString().c_str());
        return;
    }

    if (!trainer->IsTrainerValidForPlayer(_player))
    {
        WC_LOG_DEBUG("network", "WorldSession: SendTrainerList - trainer %s not valid for player %s", npc->GetGUID().ToString().c_str(), GetPlayerInfo().c_str());
        return;
    }
...
{
    WC_LOG_DEBUG("network", "WORLD: Received CMSG_TRAINER_BUY_SPELL %s, learn spell id is: %u", packet.TrainerGUID.ToString().c_str(), packet.SpellID);

    Creature* npc = GetPlayer()->GetNPCIfCanInteractWith(packet.TrainerGUID, UNIT_NPC_FLAG_TRAINER);
    if (!npc)
    {
        WC_LOG_DEBUG("network", "WORLD: HandleTrainerBuySpellOpcode - %s not found or you can not interact with him.", packet.TrainerGUID.ToString().c_str());
        return;
    }
...
{
    WC_LOG_DEBUG("network", "WORLD: Received CMSG_GOSSIP_HELLO");
    ...
    if (!unit)
    {
        WC_LOG_DEBUG("network", "WORLD: HandleGossipHelloOpcode - %s not found or you can not interact with him.", guid.ToString().c_str());
        return;
    }
...
{
    WC_LOG_DEBUG("network", "WORLD: CMSG_SPIRIT_HEALER_ACTIVATE");
    ...
    if (!unit)
    {
        WC_LOG_DEBUG("network", "WORLD: HandleSpiritHealerActivateOpcode - %s not found or you can not interact with him.", guid.ToString().c_str());
        return;
    }
...
void WorldSession::HandleBinderActivateOpcode(WorldPacket& recvData)
{
    ...
    if (!unit)
    {
        WC_LOG_DEBUG("network", "WORLD: HandleBinderActivateOpcode - %s not found or you can not interact with him.", npcGUID.ToString().c_str());
        return;
    }
...
{
    WC_LOG_DEBUG("network", "WORLD: Recv MSG_LIST_STABLED_PETS");
    ObjectGuid npcGUID;
...
void WorldSession::SendStablePetCallback(ObjectGuid guid, PreparedQueryResult result)
{
    ...
    WC_LOG_DEBUG("network", "WORLD: Recv MSG_LIST_STABLED_PETS Send.");
...
{
    WC_LOG_DEBUG("network", "WORLD: Recv CMSG_STABLE_PET");
    ObjectGuid npcGUID;
...
{
    WC_LOG_DEBUG("network", "WORLD: Recv CMSG_UNSTABLE_PET.");
    ObjectGuid npcGUID;
    uint32 petnumber;
...
{
    WC_LOG_DEBUG("network", "WORLD: Recv CMSG_BUY_STABLE_SLOT.");
    ObjectGuid npcGUID;
...
{
    WC_LOG_DEBUG("network", "HandleStableRevivePet: Not implemented");
}

void WorldSession::HandleStableSwapPet(WorldPacket& recvData)
{
    WC_LOG_DEBUG("network", "WORLD: Recv CMSG_STABLE_SWAP_PET.");
    ObjectGuid npcGUID;
    uint32 petId;
...
{
    WC_LOG_DEBUG("network", "WORLD: CMSG_REPAIR_ITEM");
    ...
    if (!unit)
    {
        WC_LOG_DEBUG("network", "WORLD: HandleRepairItemOpcode - %s not found or you can not interact with him.", npcGUID.ToString().c_str());
        return;
    }
    ...
    if (itemGUID)
    {
        WC_LOG_DEBUG("network", "ITEM: Repair %s, at %s", itemGUID.ToString().c_str(), npcGUID.ToString().c_str());
    ...
    {
        WC_LOG_DEBUG("network", "ITEM: Repair all items at %s", npcGUID.ToString().c_str());
