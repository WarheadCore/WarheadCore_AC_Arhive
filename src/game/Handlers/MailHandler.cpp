...
bool WorldSession::CanOpenMailBox(ObjectGuid guid)
{
    if (guid == _player->GetGUID())
    {
        ...
        {
            WC_LOG_WARN("cheat", "%s attempted to open mailbox by using a cheat.", _player->GetName().c_str());
            return false;
        }
    }
...
void WorldSession::HandleSendMail(WorldPacket& recvData)
{
    ...
    if (!receiverGuid)
    {
        WC_LOG_INFO("network", "Player %u is sending mail to %s (GUID: non-existing!) with subject %s "
    ...
    WC_LOG_INFO("network", "Player %u is sending mail to %s (%s) with subject %s and body %s "
    ...
    if (items_count > 0 || money > 0)
    {
        ...
        if (items_count > 0)
        {
            for (uint8 i = 0; i < items_count; ++i)
            {
                ...
                if (log)
                {
                    WC_LOG_GM(GetAccountId(), "GM %s (GUID: %u) (Account: %u) mail item: %s (Entry: %u Count: %u) "
        ...
        if (log && money > 0)
        {
            WC_LOG_GM(GetAccountId(), "GM %s (GUID: %u) (Account: %u) mail money: %u to: %s (%s) (Account: %u)",
    ...
    if (msg == EQUIP_ERR_OK)
    {
        ...
        if (m->COD > 0)                                     //if there is COD, take COD money from player and send them to sender by mail
        {
            ...
            if (HasPermission(rbac::RBAC_PERM_LOG_GM_TRADE))
            {
                ...
                WC_LOG_GM(GetAccountId(), "GM %s (Account: %u) receiver mail item: %s (Entry: %u Count: %u) and send COD money: %u to player: %s (Account: %u)",
...
void WorldSession::HandleMailCreateTextItem(WorldPacket& recvData)
{
    ...
    WC_LOG_INFO("network", "HandleMailCreateTextItem mailid=%u", mailId);
