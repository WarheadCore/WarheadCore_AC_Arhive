...
{
    WC_LOG_DEBUG("network", "WORLD: Ignore Trade %u", _player->GetGUID().GetCounter());
    // recvPacket.print_storage();
}

void WorldSession::HandleBusyTradeOpcode(WorldPacket& /*recvPacket*/)
{
    WC_LOG_DEBUG("network", "WORLD: Busy Trade %u", _player->GetGUID().GetCounter());
    // recvPacket.print_storage();
}
...
void WorldSession::moveItems(Item* myItems[], Item* hisItems[])
{
    ...
    for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
    {
        ...
        if (traderCanTrade && playerCanTrade)
        {
            ...
            {
                // logging
                WC_LOG_DEBUG("network", "partner storing: %u", myItems[i]->GetGUID().GetCounter());
                if (HasPermission(rbac::RBAC_PERM_LOG_GM_TRADE))
                {
                    WC_LOG_GM(_player->GetSession()->GetAccountId(), "GM %s (Account: %u) trade: %s (Entry: %d Count: %u) to player: %s (Account: %u)",
            ...
            {
                // logging
                WC_LOG_DEBUG("network", "player storing: %u", hisItems[i]->GetGUID().GetCounter());
                if (HasPermission(rbac::RBAC_PERM_LOG_GM_TRADE))
                {
                    WC_LOG_GM(trader->GetSession()->GetAccountId(), "GM %s (Account: %u) trade: %s (Entry: %d Count: %u) to player: %s (Account: %u)",
                        trader->GetName().c_str(), trader->GetSession()->GetAccountId(),
        ...
        else
        {
            ...
            {
                if (!traderCanTrade)
                    WC_LOG_ERROR("network", "trader can't store item: %u", myItems[i]->GetGUID().GetCounter());
                if (_player->CanStoreItem(NULL_BAG, NULL_SLOT, playerDst, myItems[i], false) == EQUIP_ERR_OK)
                    _player->MoveItemToInventory(playerDst, myItems[i], true, true);
                else
                    WC_LOG_ERROR("network", "player can't take item back: %u", myItems[i]->GetGUID().GetCounter());
            }
            // return the already removed items to the original owner
            if (hisItems[i])
            {
                if (!playerCanTrade)
                    WC_LOG_ERROR("network", "player can't store item: %u", hisItems[i]->GetGUID().GetCounter());
                if (trader->CanStoreItem(NULL_BAG, NULL_SLOT, traderDst, hisItems[i], false) == EQUIP_ERR_OK)
                    trader->MoveItemToInventory(traderDst, hisItems[i], true, true);
                else
                    WC_LOG_ERROR("network", "trader can't take item back: %u", hisItems[i]->GetGUID().GetCounter());
            }
        }
    }
}
...
static void setAcceptTradeMode(TradeData* myTrade, TradeData* hisTrade, Item* *myItems, Item* *hisItems)
{
    ...
    for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
    {
        ...
        {
            WC_LOG_DEBUG("network", "player trade item %u bag: %u slot: %u", item->GetGUID().GetCounter(), item->GetBagSlot(), item->GetSlot());
            //Can return nullptr
            myItems[i] = item;
        ...
        {
            WC_LOG_DEBUG("network", "partner trade item %u bag: %u slot: %u", item->GetGUID().GetCounter(), item->GetBagSlot(), item->GetSlot());
            hisItems[i] = item;
...
void WorldSession::HandleAcceptTradeOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    if (his_trade->IsAccepted())
    {
        ...
        if (HasPermission(rbac::RBAC_PERM_LOG_GM_TRADE))
        {
            ...
            {
                WC_LOG_GM(_player->GetSession()->GetAccountId(), "GM %s (Account: %u) give money (Amount: %u) to player: %s (Account: %u)",
                    _player->GetName().c_str(), _player->GetSession()->GetAccountId(),
                    my_trade->GetMoney(),
                    trader->GetName().c_str(), trader->GetSession()->GetAccountId());
            }
            ...
            {
                WC_LOG_GM(trader->GetSession()->GetAccountId(), "GM %s (Account: %u) give money (Amount: %u) to player: %s (Account: %u)",
                    trader->GetName().c_str(), trader->GetSession()->GetAccountId(),
                    his_trade->GetMoney(),
                    _player->GetName().c_str(), _player->GetSession()->GetAccountId());
            }
