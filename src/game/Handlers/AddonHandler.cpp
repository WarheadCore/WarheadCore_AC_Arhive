    ...
    *source >> TempValue;                                   // get real size of the packed structure
    ...
    if (uncompress(AddOnPacked.contents(), &AddonRealSize, source->contents() + CurrentPosition, source->size() - CurrentPosition) == Z_OK)
    {
        ...
        for (uint32 i = 0; i < addonsCount; ++i)
        {
            ...
            WC_LOG_DEBUG("network", "ADDON: Name: %s, Enabled: 0x%x, CRC: 0x%x, Unknown2: 0x%x", addonName.c_str(), enabled, crc, unk2);
        ...
        *target << uint32(count);
        ...
        if (AddOnPacked.rpos() != AddOnPacked.size())
            WC_LOG_DEBUG("network", "packet under read!");
    }
    else
    {
        WC_LOG_ERROR("network", "Addon packet uncompress error :(");
        return false;
    }
    return true;
}
