...
void CharacterDatabaseCleaner::CleanDatabase()
{
    ...
    WC_LOG_INFO("misc", "Cleaning character database...");
    ...
    WC_LOG_INFO("server.loading", ">> Cleaned character database in %u ms", GetMSTimeDiffToNow(oldMSTime));
}
...
void CharacterDatabaseCleaner::CheckUnique(char const* column, char const* table, bool (*check)(uint32))
{
    ...
    if (!result)
    {
        WC_LOG_INFO("misc", "Table %s is empty.", table);
        return;
    }
