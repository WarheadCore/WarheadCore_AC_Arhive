    ...
    struct npc_love_in_air_supply_sentryAI : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* who)
        {
            if (lock > 1000 && me->GetDistance(who) < 10.0f && who->GetTypeId() == TYPEID_PLAYER && who->HasAura(SPELL_GOBLIN_DISGUISE) && !who->HasAura(SPELL_GOBLIN_CARRY_CRATE))
            {
                lock = 0;
                if (urand(0, 1))
                    me->Say("Time is money, friend. Go go go!", LANG_UNIVERSAL, who->ToPlayer());
                else
                    me->Say("That crate won't deliver itself, friend. Get a move on!", LANG_UNIVERSAL, who->ToPlayer());
                ...
                case NPC_SNIVEL_HORDE:
                    {
                        ...
                        {
                            case 1:
                                me->Say("What are you staring at? Haven't you ever seen a genius before?", LANG_UNIVERSAL);
                                return false;
                            case 2:
                                me->Say("This'll teach you to mind your own business!", LANG_UNIVERSAL);
                                return true;
                        }
                        break;
                    }
                ...
                case NPC_SNIVEL_HORDE+1:
                    {
                        ...
                        {
                            case 1:
                                me->Say("That's right. I'd like to list some of these 'fireworks'.", LANG_UNIVERSAL);
                                return false;
                            case 2:
                                me->Say("Those'll net me a nice profit when I return from the South Seas.", LANG_UNIVERSAL);
                                return false;
                            case 3:
                                me->Say("You... Don't think I don't see you. Leave me alone!", LANG_UNIVERSAL);
                                return true;
                        }
                        break;
                    }
                ...
                case NPC_SNIVEL_HORDE+2:
                    {
                        ...
                        {
                            case 1:
                                me->Say("Thanks for the great cut and shave, buddy.", LANG_UNIVERSAL);
                                return false;
                            case 2:
                                me->Say("Here's a little something extra since I'll be away.", LANG_UNIVERSAL);
                                return false;
                            case 3:
                                me->Say("On second thought, keep the whole bag. I have to, uh, get going.", LANG_UNIVERSAL);
                                return false;
                            case 4:
                                me->Say("Did you really think you could corner me this easily?", LANG_UNIVERSAL);
                                return true;
                        }
                        break;
                    }
...
public:
    ...
    struct npc_love_in_air_hummelAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* )
        {
            me->Say("...please don't think less of me.", LANG_UNIVERSAL);
        ...
        void UpdateAI(uint32 diff)
        {
            if (speachTimer)
            {
                ...
                {
                    me->Say("Did they bother to tell you who I am and why I am doing this?", LANG_UNIVERSAL);
                    speachTimer = 10000;
                }
                else if (speachTimer >= 16000 && speachTimer < 20000)
                {
                    me->Say("...or are they just using you like they do everybody else?", LANG_UNIVERSAL);
                    speachTimer = 20000;
                }
                else if (speachTimer >= 26000 && speachTimer < 30000)
                {
                    me->Say("But what does it matter. It is time for this to end.", LANG_UNIVERSAL);
                    speachTimer = 0;
                    me->setFaction(16);
...
public:
    ...
    struct npc_love_in_air_hummel_helperAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* ) { me->Say("...please don't think less of me.", LANG_UNIVERSAL); }
