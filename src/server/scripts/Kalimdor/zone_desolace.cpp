        ...
        struct npc_cork_gizeltonAI : public npc_escortAI
        {
            ...
            void WaypointReached(uint32 waypointId)
            {
                ...
                switch (waypointId)
                {
                    // Finished north path
                    case 52:
                        me->SummonCreature(NPC_VENDOR_TRON, -694.61f, 1460.7f, 90.794f, 2.4f, TEMPSUMMON_TIMED_DESPAWN, Milliseconds(TIME_SHOP_STOP).count() + 15 * IN_MILLISECONDS);
                    ...
                    // Finished south path
                    case 193:
                        me->SummonCreature(NPC_SUPER_SELLER, -1905.5f, 2463.3f, 61.52f, 5.87f, TEMPSUMMON_TIMED_DESPAWN, Milliseconds(TIME_SHOP_STOP).count() + 15 * IN_MILLISECONDS);
                        SetEscortPaused(true);
                        events.ScheduleEvent(EVENT_RESUME_PATH, TIME_SHOP_STOP);
                        CheckCaravan();
                        break;
