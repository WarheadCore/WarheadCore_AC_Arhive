...
public:
    ...
    struct npc_eye_of_acherusAI : public NullCreatureAI
    {
        ...
        {
            events.Reset();
            events.ScheduleEvent(EVENT_REMOVE_CONTROL, 500ms);
            events.ScheduleEvent(EVENT_SPEAK_1, 4s);
            events.ScheduleEvent(EVENT_LAUNCH, 7s);
        ...
        void MovementInform(uint32 type, uint32 point) override
        {
            if (type == ESCORT_MOTION_TYPE || point !=0)
                events.ScheduleEvent(EVENT_REGAIN_CONTROL, 1s);
        }
...
public:
    ...
    struct npc_scarlet_ghoulAI : public ScriptedAI
    {
        ...
        void InitializeAI() override
        {
            ...
            events.ScheduleEvent(EVENT_GHOUL_EMOTE, 1s);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case PHASE_TO_EQUIP:
                if (wait_timer)
                {
                    ...
                    else
                    {
                        ...
                        //LOG_DEBUG("tscr", "npc_unworthy_initiateAI: move to %f %f %f", anchorX, anchorY, me->GetPositionZ());
            ...
            case PHASE_ATTACKING:
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    events.ScheduleEvent(eventId, 5s, GCD_CAST);
                }
