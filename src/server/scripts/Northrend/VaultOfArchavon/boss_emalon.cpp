            ...
            switch (events.ExecuteEvent())
            {
                case EVENT_CHAIN_LIGHTNING:
                    if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                        me->CastSpell(target, RAID_MODE_HEROIC(SPELL_CHAIN_LIGHTNING_10, SPELL_CHAIN_LIGHTNING_25), false);
                    events.RepeatEvent(25s);
                    break;
                case EVENT_LIGHTNING_NOVA:
                    me->CastSpell(me, RAID_MODE_HEROIC(SPELL_LIGHTNING_NOVA_10, SPELL_LIGHTNING_NOVA_25), false);
