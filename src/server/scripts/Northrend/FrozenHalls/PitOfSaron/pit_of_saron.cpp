...
public:
    ...
    struct npc_pos_leader_secondAI: public NullCreatureAI
    {
        ...
        void DoAction(int32 p)
        {
            if (p == 1)
            {
                events.RescheduleEvent(1, me->GetEntry() == NPC_JAINA_PART2 ? 15500ms : 18s);
                events.RescheduleEvent(2, me->GetEntry() == NPC_JAINA_PART2 ? 16500ms : 19s);
            }
        }
