        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                case EVENT_SPELL_IMPALE:
                    if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                        me->CastSpell(target, RAID_MODE_HEROIC(SPELL_IMPALE_10, SPELL_IMPALE_25), false);
                    events.RepeatEvent(20s);
                    break;
                case EVENT_SPELL_LOCUST_SWARM:
                    {
                        Talk(EMOTE_LOCUST);
                        me->CastSpell(me, RAID_MODE_HEROIC(SPELL_LOCUST_SWARM_10, SPELL_LOCUST_SWARM_25), false);
                        Position pos;
