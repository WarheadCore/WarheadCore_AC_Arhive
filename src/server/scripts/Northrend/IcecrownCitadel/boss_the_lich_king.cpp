            ...
            void UpdateAI(uint32 diff)
            {
                ...
                switch (events.ExecuteEvent())
                {
                    ...
                    case EVENT_INFEST:
                        ...
                        events.ScheduleEvent(EVENT_INFEST, 22500ms, EVENT_GROUP_ABILITIES);
                        break;
                    ...
                    case EVENT_PAIN_AND_SUFFERING:
                        ...
                        events.ScheduleEvent(EVENT_PAIN_AND_SUFFERING, IsHeroic() ? 1250ms : 1750ms, IsHeroic() ? 1750ms : 2250ms, EVENT_GROUP_ABILITIES);
                        break;
                    ...
                    case EVENT_SOUL_REAPER:
                        if (me->IsWithinMeleeRange(me->GetVictim()))
                        {
                            ...
                            events.ScheduleEvent(EVENT_SOUL_REAPER, 30500ms, EVENT_GROUP_ABILITIES);
                        }
                        else
            ...
            void MovementInform(uint32 type, uint32 id)
            {
                ...
                switch (id)
                {
                    ...
                    case POINT_TIRION_OUTRO:
                        _events.ScheduleEvent(EVENT_OUTRO_FORDRING_JUMP, 10s);
                        break;
                }
            }
            ...
            void DoAction(int32 action)
            {
                switch (action)
                {
                    case ACTION_OUTRO:
                        ...
                        _events.ScheduleEvent(EVENT_OUTRO_LK_EMOTE_TALK, 17600ms);
                ...
                switch (_events.ExecuteEvent())
                {
                    ...
                    case EVENT_OUTRO_LK_MOVE_CENTER:
                        if (Creature* theLichKing = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_THE_LICH_KING)))
                        {
                            ...
                            _events.ScheduleEvent(EVENT_OUTRO_LK_TALK_4, 10ms + Milliseconds(travelTime));
                            _events.ScheduleEvent(EVENT_OUTRO_LK_RAISE_DEAD, 1s + Milliseconds(travelTime));
                            _events.ScheduleEvent(EVENT_OUTRO_LK_TALK_5, 29s + Milliseconds(travelTime));
                        }
                        break;
    ...
    public:
        ...
        struct npc_valkyr_shadowguardAI : public NullCreatureAI
        {
            ...
            void GoSiphon()
            {
                ...
                _events.Reset();
                _events.ScheduleEvent(EVENT_MOVE_TO_SIPHON_POS, 0s);
            }
                    ...
                    case POINT_DROP_PLAYER:
                        {
                            ...
                            {
                                _events.Reset();
                                _events.ScheduleEvent(EVENT_MOVE_TO_DROP_POS, 0s);
                                break;
                            }
                            dropped = true;
                            _events.Reset();
