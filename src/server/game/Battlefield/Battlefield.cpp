...
void Battlefield::InitStalker(uint32 entry, float x, float y, float z, float o)
{
    if (Creature* creature = SpawnCreature(entry, x, y, z, o, TEAM_NEUTRAL))
        ...
        LOG_ERROR("auctionHouse", "Battlefield::InitStalker: could not spawn Stalker (Creature entry %u), zone messeges will be un-available", entry);
}
...
GameObject* Battlefield::SpawnGameObject(uint32 entry, float x, float y, float z, float o)
{
    ...
    if (!go->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_GAMEOBJECT), entry, map, PHASEMASK_NORMAL, x, y, z, o, QuaternionData(), 100, GO_STATE_READY))
    {
...
bool BfCapturePoint::SetCapturePointData(GameObject* capturePoint)
{
    ...
    LOG_DEBUG("bg.bf", "Creating capture point %u", capturePoint->GetEntry());
