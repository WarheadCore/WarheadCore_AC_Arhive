...
void AuctionHouseMgr::SendAuctionSuccessfulMail(AuctionEntry* auction, SQLTransaction& trans, bool sendNotification, bool updateAchievementCriteria, bool sendMail)
{
    ...
    if (owner || owner_accId)
    {
        ...
        if (sendMail) // can be changed in the hook
            ...
            .AddMoney(profit)
            .SendMailTo(trans, MailReceiver(owner, auction->owner), auction, MAIL_CHECK_MASK_COPIED, CONF_GET_INT("MailDeliveryDelay"));
...
void AuctionHouseMgr::LoadAuctionItems()
{
    ...
    if (!result)
    {
        LOG_WARN("sql.sql", ">> Loaded 0 auction items. DB table `auctionhouse` or `item_instance` is empty!");
        LOG_WARN("sql.sql", "");
        return;
    }
...
void AuctionHouseMgr::LoadAuctions()
{
    ...
    if (!result)
    {
        LOG_WARN("sql.sql", ">> Loaded 0 auctions. DB table `auctionhouse` is empty.");
        LOG_WARN("sql.sql", "");
        return;
    }
