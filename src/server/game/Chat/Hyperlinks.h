    ...
    namespace Hyperlinks
    {
        ...
        namespace LinkTags
        {
            ...
            struct base_tag
            {
                ...
                static bool StoreTo(uint64& val, char const* pos, size_t len)
                {
                    try { std::stoul(std::string(pos, len), nullptr, 16); }
                    catch (...) { return false; }
                    return true;
                }
            };
...
#undef make_base_tag
        ...
        bool CheckAllLinks(std::string const&);
