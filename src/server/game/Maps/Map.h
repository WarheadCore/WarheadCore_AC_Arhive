...
public:
    ...
    std::mutex& GetMMapLock() const {
        return *(const_cast<std::mutex*>(&MMapLock));
    }
    // pussywizard:
    ...
    virtual bool CanEnter(Player* /*player*/, bool /*loginCheck = false*/) {
        return true;
    }
    ...
    bool Is25ManRaid() const {
        return IsRaid() && i_spawnMode & RAID_DIFFICULTY_MASK_25MAN;    // since 25man difficulties are 1 and 3, we can check them like that
    }
