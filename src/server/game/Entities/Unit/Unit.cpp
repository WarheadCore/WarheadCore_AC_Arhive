...
uint32 Unit::DealDamage(Unit* attacker, Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss, bool  /*allowGM*/)
{
    ...
    LOG_INFO("entities.unit", "deal dmg:%d to health:%d ", damage, health);
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    {
        // Roll dodge
        int32 dodgeChance = int32(victim->GetUnitDodgeChance(attType, victim) * 100.0f) - skillDiff * 4;
    ...
    {
        // Roll parry
        int32 parryChance = int32(victim->GetUnitParryChance(attType, victim) * 100.0f)  - skillDiff * 4;
    ...
    if (canBlock)
    {
        int32 blockChance = int32(victim->GetUnitBlockChance(attType, victim) * 100.0f) - skillDiff * 4;
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    if (triggerEntry == nullptr)
    {
        ...
        LOG_ERROR("entities.unit", "Unit::HandleProcTriggerSpell: Spell %u (effIndex: %u) has unknown TriggerSpell %u. Unhandled custom case?",
...
bool Unit::IsImmunedToSpellEffect(SpellInfo const* spellInfo, uint32 index) const
{
    ...
    {
        SpellImmuneList const& mechanicList = m_spellImmune[IMMUNITY_MECHANIC];
        for (auto const& itr : mechanicList)
        {
    ...
    if (!spellInfo->HasAttribute(SPELL_ATTR3_IGNORE_HIT_RESULT))
    {
        if (uint32 aura = spellInfo->Effects[index].ApplyAuraName)
        {
            ...
            if (!spellInfo->HasAttribute(SPELL_ATTR2_UNAFFECTED_BY_AURA_SCHOOL_IMMUNE))
            {
                ...
                for (auto const& iter : immuneAuraApply)
                {
                    if (((*iter).GetMiscValue() & spellInfo->GetSchoolMask()) && !spellInfo->IsPositiveEffect(index) && spellInfo->Effects[index].Effect != SPELL_EFFECT_PERSISTENT_AREA_AURA)
                        return true;
                }
            }
        }
...
void Unit::Kill(Unit* killer, Unit* victim, bool durabilityLoss, WeaponAttackType attackType, SpellInfo const* spellProto)
{
    ...
    if (Player* plrVictim = victim->ToPlayer())
    {
        ...
        {
            LOG_DEBUG("entities.unit", "We are dead, losing %f percent durability", sGameConfig->GetFloatConfig("DurabilityLoss.OnDeath"));
...
void Unit::OutDebugInfo() const
{
    ...
    for (uint32 i = 0; i < MAX_SUMMON_SLOT; ++i)
        LOG_INFO("entities.unit", UI64FMTD", ", m_SummonSlot[i]);
    LOG_INFO("entities.unit", "");

    LOG_INFO("entities.unit", "Controlled List: ");
    for (ControlSet::const_iterator itr = m_Controlled.begin(); itr != m_Controlled.end(); ++itr)
        LOG_INFO("entities.unit", UI64FMTD", ", (*itr)->GetGUID());
    LOG_INFO("entities.unit", "");
    ...
    if (IsVehicle())
    {
        ...
        for (SeatMap::iterator itr = GetVehicleKit()->Seats.begin(); itr != GetVehicleKit()->Seats.end(); ++itr)
            if (Unit* passenger = ObjectAccessor::GetUnit(*GetVehicleBase(), itr->second.Passenger.Guid))
                LOG_INFO("entities.unit", UI64FMTD", ", passenger->GetGUID());
        LOG_INFO("entities.unit", "");
    }
...
{
    Talk(textId, CHAT_MSG_MONSTER_SAY, sGameConfig->GetFloatConfig("ListenRange.Say"), target);
}

void Unit::Yell(uint32 textId, WorldObject const* target /*= nullptr*/)
{
    Talk(textId, CHAT_MSG_MONSTER_YELL, sGameConfig->GetFloatConfig("ListenRange.Yell"), target);
}

void Unit::TextEmote(uint32 textId, WorldObject const* target /*= nullptr*/, bool isBossEmote /*= false*/)
{
    Talk(textId, isBossEmote ? CHAT_MSG_RAID_BOSS_EMOTE : CHAT_MSG_MONSTER_EMOTE, sGameConfig->GetFloatConfig("ListenRange.TextEmote"), target);
}
