...
bool Corpse::Create(uint32 guidlow, Player* owner)
{
    ...
    if (!IsPositionValid())
    {
        LOG_ERROR("entities.corpse", "Corpse (guidlow %d, owner %s) not created. Suggested coordinates isn't valid (X: %f Y: %f)",
...
bool Corpse::LoadCorpseFromDB(uint32 guid, Field* fields)
{
    ...
    if (!_LoadIntoDataField(fields[6].GetString(), CORPSE_FIELD_ITEM, EQUIPMENT_SLOT_END))
    {
        LOG_ERROR("entities.player", "Corpse (%u, owner: %u) is not created, given equipment info is not valid ('%s')",
                  GetGUIDLow(), static_cast<uint32>(GetOwnerGUID()), fields[6].GetString().c_str());
    }
    ...
    if (!IsPositionValid())
    {
        LOG_ERROR("entities.corpse", "Corpse (guid: %u, owner: %u) is not created, given coordinates are not valid (X: %f, Y: %f, Z: %f)",
