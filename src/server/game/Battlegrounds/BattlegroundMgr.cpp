...
void BattlegroundMgr::CreateInitialBattlegrounds()
{
    ...
    if (!result)
    {
        LOG_WARN("sql.sql", ">> Loaded 0 battlegrounds. DB table `battleground_template` is empty.");
        LOG_WARN("sql.sql", "");
        return;
    }
...
void BattlegroundMgr::InitAutomaticArenaPointDistribution()
{
    ...
    LOG_INFO("bg.battleground", "Battleground: Initializing Automatic Arena Point Distribution");

    if (wstime < curtime)
    {
        m_NextAutoDistributionTime = curtime;           // reset will be called in the next update
        LOG_INFO("bg.battleground", "Battleground: Next arena point distribution time in the past, reseting it now.");
    }
    else
        m_NextAutoDistributionTime = wstime;

    LOG_INFO("bg.battleground", "Battleground: Automatic Arena Point Distribution initialized.");
}
...
{
    m_Testing = CONF_GET_BOOL("Debug.Battleground") ? true : !m_Testing;
    sWorld->SendWorldText(m_Testing ? LANG_DEBUG_BG_ON : LANG_DEBUG_BG_OFF);
}

void BattlegroundMgr::ToggleArenaTesting()
{
    m_ArenaTesting = CONF_GET_BOOL("Debug.Arena") ? true : !m_ArenaTesting;
...
void BattlegroundMgr::LoadBattleMastersEntry()
{
    ...
    if (!result)
    {
        LOG_WARN("sql.sql", ">> Loaded 0 battlemaster entries. DB table `battlemaster_entry` is empty!");
        LOG_WARN("sql.sql", "");
        return;
    }
