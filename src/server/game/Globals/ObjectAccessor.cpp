...
{
    WARHEAD_READ_GUARD(ACE_RW_Thread_Mutex, i_corpseLock);
...
void ObjectAccessor::RemoveCorpse(Corpse* corpse, bool final)
{
    ...
    if (!final)
    {
        WARHEAD_WRITE_GUARD(ACE_RW_Thread_Mutex, i_corpseLock);
...
Corpse* ObjectAccessor::ConvertCorpseForPlayer(uint64 player_guid, bool insignia /*=false*/)
{
    ...
    if (map && corpse->IsPositionValid() && inWorld && (insignia ||
            (map->IsBattlegroundOrArena() ? CONF_GET_BOOL("Death.Bones.BattlegroundOrArena") : CONF_GET_BOOL("Death.Bones.World"))) &&
...
{
    WARHEAD_GUARD(ACE_Thread_Mutex, DelayedCorpseLock);
