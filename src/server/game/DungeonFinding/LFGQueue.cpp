...
namespace lfg
{
    ...
    {
        //LOG_INFO("lfg", "ADD AddToQueue: %u, failed proposal: %u", GUID_LOPART(guid), failedProposal ? 1 : 0);
        ...
        //LOG_INFO("lfg", "AddToQueue success: %u", GUID_LOPART(guid));
        AddToNewQueue(guid, failedProposal);
    }

    void LFGQueue::RemoveFromQueue(uint64 guid, bool partial)
    {
        //LOG_INFO("lfg", "REMOVE RemoveFromQueue: %u, partial: %u", GUID_LOPART(guid), partial ? 1 : 0);
        ...
        for (LfgQueueDataContainer::iterator itr = QueueDataStore.begin(); itr != QueueDataStore.end(); ++itr)
        {
            if (itr->first != guid)
            {
                ...
                {
                    //LOG_INFO("lfg", "CLEAR bestCompatible: %s, because of guid: %u", itr->second.bestCompatible.toString().c_str(), GUID_LOPART(guid));
                    itr->second.bestCompatible.clear();
                }
            }
            else
            {
                //LOG_INFO("lfg", "CLEAR bestCompatible SELF: %s, because of guid: %u", itr->second.bestCompatible.toString().c_str(), GUID_LOPART(guid));
        ...
        {
            //LOG_INFO("lfg", "ERASE QueueDataStore for: %u", GUID_LOPART(guid));
            //LOG_INFO("lfg", "ERASE QueueDataStore for: %u, itDelete: %u,%u,%u", GUID_LOPART(guid), itDelete->second.dps, itDelete->second.healers, itDelete->second.tanks);
            QueueDataStore.erase(itDelete);
            //LOG_INFO("lfg", "ERASE QueueDataStore for: %u SUCCESS", GUID_LOPART(guid));
        }
    }
    ...
    void LFGQueue::AddToNewQueue(uint64 guid, bool front)
    {
        if (front)
        {
            //LOG_INFO("lfg", "ADD AddToNewQueue at FRONT: %u", GUID_LOPART(guid));
            restoredAfterProposal.push_back(guid);
            newToQueueStore.push_front(guid);
        }
        else
        {
            //LOG_INFO("lfg", "ADD AddToNewQueue at the END: %u", GUID_LOPART(guid));
    ...
    {
        //LOG_INFO("lfg", "REMOVE RemoveFromNewQueue: %u", GUID_LOPART(guid));
    ...
    {
        //LOG_INFO("lfg", "JOINED AddQueueData: %u", GUID_LOPART(guid));
    ...
    {
        //LOG_INFO("lfg", "LEFT RemoveQueueData: %u", GUID_LOPART(guid));
    ...
    {
        //LOG_INFO("lfg", "COMPATIBLES REMOVE for: %u", GUID_LOPART(guid));
        for (LfgCompatibleContainer::iterator it = CompatibleList.begin(); it != CompatibleList.end(); ++it)
            if (it->hasGuid(guid))
            {
                //LOG_INFO("lfg", "Removed Compatible: %s, because of guid: %u", it->toString().c_str(), GUID_LOPART(guid));
        ...
        for (LfgCompatibleContainer::iterator itr = CompatibleTempList.begin(); itr != CompatibleTempList.end(); )
        {
            ...
            if (it->hasGuid(guid))
            {
                //LOG_INFO("lfg", "Erased Temp Compatible: %s, because of guid: %u", it->toString().c_str(), GUID_LOPART(guid));
    ...
    {
        //LOG_INFO("lfg", "COMPATIBLES ADD: %s", key.toString().c_str());
        CompatibleTempList.push_back(key);
    }

    uint8 LFGQueue::FindGroups()
    {
        //LOG_INFO("lfg", "FIND GROUPS!");
        ...
        if (!newToQueueStore.empty())
        {
            ...
            //LOG_INFO("lfg", "newToQueueStore guid: %u, front: %u", GUID_LOPART(newGuid), pushCompatiblesToFront ? 1 : 0);
    ...
    LfgCompatibility LFGQueue::FindNewGroups(const uint64& newGuid)
    {
        ...
        //LOG_INFO("lfg", "FIND NEW GROUPS for: %u", GUID_LOPART(newGuid));
        ...
        for (Lfg5GuidsList::iterator it = CompatibleList.begin(); it != CompatibleList.end(); )
        {
            ...
            if (itr->empty())
            {
                //LOG_INFO("lfg", "ERASE from CompatibleList");
    ...
    {
        //LOG_INFO("lfg", "CHECK CheckCompatibility: %s, new guid: %u", checkWith.toString().c_str(), GUID_LOPART(newGuid));
    ...
    void LFGQueue::UpdateQueueTimers(uint32 diff)
    {
        ...
        //LOG_INFO("lfg", "UPDATE UpdateQueueTimers");
        for (Lfg5GuidsList::iterator it = CompatibleList.begin(); it != CompatibleList.end(); )
        {
            Lfg5GuidsList::iterator itr = it++;
            if (itr->empty())
            {
                //LOG_INFO("lfg", "UpdateQueueTimers ERASE compatible");
                CompatibleList.erase(itr);
            }
        }
        ...
        for (LfgQueueDataContainer::iterator itQueue = QueueDataStore.begin(); itQueue != QueueDataStore.end(); ++itQueue)
        {
            ...
            {
                //LOG_INFO("lfg", "found empty bestCompatible");
    ...
    {
        //LOG_INFO("lfg", "UpdateBestCompatibleInQueue: %s", key.toString().c_str());
