...
inline void LoadDBC(uint32& availableDbcLocales, StoreProblemList& errors, DBCStorage<T>& storage, std::string const& dbcPath, std::string const& filename, char const* dbTable = nullptr)
{
    ...
    LOG_INFO("dbc", "Load dbc '%s'", filename.c_str());
...
#undef LOAD_DBC
    ...
    else if (!bad_dbc_files.empty())
    {
        ...
        LOG_ERROR("dbc", "Some required *.dbc files (%u from %u) not found or not compatible:\n %s", static_cast<uint32>(bad_dbc_files.size()), DBCFileCount, str.c_str());
        exit(1);
    }
    ...
    LOG_INFO("dbc", ">> Initialized %d data stores in %u ms", DBCFileCount, GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("dbc", "");
}
