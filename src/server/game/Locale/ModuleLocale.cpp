...
void ModuleLocale::CheckStrings(std::string const& moduleName, uint8 maxString)
{
    uint32 stringCount = sModuleLocale->GetStringsCount(moduleName);

    if (stringCount != maxString - 1)
        LOG_FATAL("locale.module", "> Strings locale (%u) for module(%s) != (%u)", stringCount, moduleName.c_str(), maxString - 1);
}
