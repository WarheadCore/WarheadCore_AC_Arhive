...
bool DBUpdaterUtil::CheckExecutable()
{
    ...
    if (!exists(exe))
    {
        ...
        WC_LOG_FATAL("sql.updates", "Didn't find any executable MySQL binary at \'%s\' or in path, correct the path in the *.conf (\"MySQLExecutable\").",
...
{
    WC_LOG_INFO("sql.updates", "Database \"%s\" does not exist, do you want to create it? [yes (default) / no]: ",
    ...
    WC_LOG_INFO("sql.updates", "Creating database \"%s\"...", pool.GetConnectionInfo()->database.c_str());
    ...
    if (!file.is_open())
    {
        WC_LOG_FATAL("sql.updates", "Failed to create temporary query file \"%s\"!", temp.generic_string().c_str());
        return false;
    }
    ...
    {
        WC_LOG_FATAL("sql.updates", "Failed to create database %s! Does the user (named in *.conf) have `CREATE`, `ALTER`, `DROP`, `INSERT` and `DELETE` privileges on the MySQL server?", pool.GetConnectionInfo()->database.c_str());
        boost::filesystem::remove(temp);
        return false;
    }

    WC_LOG_INFO("sql.updates", "Done.");
    boost::filesystem::remove(temp);
    return true;
}
...
bool DBUpdater<T>::Update(DatabaseWorkerPool<T>& pool)
{
    ...
    WC_LOG_INFO("sql.updates", "Updating %s database...", DBUpdater<T>::GetTableName().c_str());

    Path const sourceDirectory(BuiltInConfig::GetSourceDirectory());

    if (!is_directory(sourceDirectory))
    {
        WC_LOG_ERROR("sql.updates", "DBUpdater: The given source directory %s does not exist, change the path to the directory where your sql directory exists (for example c:\\source\\trinitycore). Shutting down.", sourceDirectory.generic_string().c_str());
        return false;
    }
    ...
    if (!result.updated)
        WC_LOG_INFO("sql.updates", ">> %s database is up-to-date! %s", DBUpdater<T>::GetTableName().c_str(), info.c_str());
    else
        WC_LOG_INFO("sql.updates", ">> Applied " SZFMTD " %s. %s", result.updated, result.updated == 1 ? "query" : "queries", info.c_str());
...
bool DBUpdater<T>::Populate(DatabaseWorkerPool<T>& pool)
{
    ...
    WC_LOG_INFO("sql.updates", "Database %s is empty, auto populating it...", DBUpdater<T>::GetTableName().c_str());

    std::string const p = DBUpdater<T>::GetBaseFile();
    if (p.empty())
    {
        WC_LOG_INFO("sql.updates", ">> No base file provided, skipped!");
        return true;
    }
    ...
    if (!exists(base))
    {
        switch (DBUpdater<T>::GetBaseLocationType())
        {
            ...
            {
                WC_LOG_ERROR("sql.updates", ">> Base file \"%s\" is missing. Try fixing it by cloning the source again.",
            ...
            case LOCATION_DOWNLOAD:
            {
                ...
                WC_LOG_ERROR("sql.updates", ">> File \"%s\" is missing, download it from \"https://github.com/TrinityCore/TrinityCore/releases\"" \
    ...
    // Update database
    WC_LOG_INFO("sql.updates", ">> Applying \'%s\'...", base.generic_string().c_str());
    try
    {
        ApplyFile(pool, base);
    }
    ...
    std::string const& password, std::string const& port_or_socket, std::string const& database, Path const& path)
{
    ...
    if (ret != EXIT_SUCCESS)
    {
        WC_LOG_FATAL("sql.updates", "Applying of file \'%s\' to database \'%s\' failed!" \
...
template class WC_DATABASE_API DBUpdater<LoginDatabaseConnection>;
template class WC_DATABASE_API DBUpdater<WorldDatabaseConnection>;
template class WC_DATABASE_API DBUpdater<CharacterDatabaseConnection>;
