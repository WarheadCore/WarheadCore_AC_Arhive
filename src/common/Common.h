...
#define WARHEAD_GUARD(MUTEX, LOCK) \
  ACE_Guard< MUTEX > WARHEAD_GUARD_OBJECT (LOCK); \
...
# define WARHEAD_WRITE_GUARD(MUTEX, LOCK) \
  ACE_Write_Guard< MUTEX > WARHEAD_GUARD_OBJECT (LOCK); \
    if (WARHEAD_GUARD_OBJECT.locked() == 0) ABORT();

//! For proper implementation of multiple-read, single-write pattern, use
//! ACE_RW_Mutex as underlying @MUTEX
# define WARHEAD_READ_GUARD(MUTEX, LOCK) \
