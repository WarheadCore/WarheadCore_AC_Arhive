/*
 * Copyright (C) 2008-2019 WarheadCore <https://www.WarheadCore.org/>
...
int main(int argc, char** argv)
{
    ...
    Warhead::Banner::Show("authserver",
        ...
        []()
        {
            WC_LOG_INFO("server.authserver", "Using configuration file %s.", sConfigMgr->GetFilename().c_str());
            WC_LOG_INFO("server.authserver", "Using SSL version: %s (library: %s)", OPENSSL_VERSION_TEXT, SSLeay_version(SSLEAY_VERSION));
            WC_LOG_INFO("server.authserver", "Using Boost version: %i.%i.%i", BOOST_VERSION / 100000, BOOST_VERSION / 100 % 1000, BOOST_VERSION % 100);
        }
    );
    ...
    if (!pidFile.empty())
    {
        if (uint32 pid = CreatePIDFile(pidFile))
            WC_LOG_INFO("server.authserver", "Daemon PID: %u\n", pid);
        else
        {
            WC_LOG_ERROR("server.authserver", "Cannot create PID file %s.\n", pidFile.c_str());
            return 1;
        }
    }
    ...
    {
        WC_LOG_ERROR("server.authserver", "No valid realms specified.");
        return 1;
    }
    ...
    {
        WC_LOG_ERROR("server.authserver", "Specified port out of allowed range (1-65535)");
        return 1;
    }
    ...
    {
        WC_LOG_ERROR("server.authserver", "Failed to initialize network");
        return 1;
    }
    ...
    WC_LOG_INFO("server.authserver", "Halting process...");
...
void KeepDatabaseAliveHandler(std::weak_ptr<Warhead::Asio::DeadlineTimer> dbPingTimerRef, int32 dbPingInterval, boost::system::error_code const& error)
{
    if (!error)
    {
        ...
        {
            WC_LOG_INFO("server.authserver", "Ping MySQL to keep connection alive");
