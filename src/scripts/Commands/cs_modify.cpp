...
public:
    ...
    static bool HandleModifyEnergyCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (CheckModifyResources(handler, args, target, energy, energymax, energyMultiplier))
        {
            ...
            WC_LOG_DEBUG("misc", handler->GetTrinityString(LANG_CURRENT_ENERGY), target->GetMaxPower(POWER_ENERGY));
            return true;
        }
        return false;
    }
    ...
    static bool HandleModifyMoneyCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (moneyToAdd < 0)
        {
            ...
            WC_LOG_DEBUG("misc", handler->GetTrinityString(LANG_CURRENT_MONEY), targetMoney, moneyToAdd, newmoney);
            if (newmoney <= 0)
            {
        ...
        WC_LOG_DEBUG("misc", handler->GetTrinityString(LANG_NEW_MONEY), targetMoney, moneyToAdd, target->GetMoney());
